...
/* ─── Types ─────────────────────────────────────────────── */
type ProductId = "kendaraan" | "properti" | "liability" | "engineering" | "surety" | "cargo";
...
const FIELD_CONFIGS: Record<ProductId, FieldConfig[]> = {
  ...
  properti: [
    ...
    { id: "okupasi", label: "Okupasi / Penggunaan", type: "select",
    ...
    { id: "risikoTambahan", label: "Risiko Tambahan", type: "multicheck",
  ...
  engineering: [
    { id: "jenisProyek", label: "Jenis Proyek / Produk", type: "select",
...
const WA_NUMBER = "6287781658231";

/* ─── Sub-components ─────────────────────────────────────── */
const inputBase =
  "w-full bg-white border border-[#D4C9B8] rounded-lg px-3.5 py-2.5 text-sm text-navy placeholder-[#94A3B8] " +
...
export default function SPPAForm() {
  ...
  const [form, setForm] = useState<FormState>({ nama: "", whatsapp: "", agree: false });
  ...
  const handleProductChange = (val: string) => {
    ...
    setForm({ nama: form.nama, whatsapp: form.whatsapp, agree: false });
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!product) return alert("Silakan pilih produk asuransi terlebih dahulu.");
    if (!form.nama || !form.whatsapp) return alert("Nama dan nomor WhatsApp wajib diisi.");
    if (!form.agree) return alert("Harap centang persetujuan sebelum mengirim.");
    ...
    msg += `\n\n_Dikirim via form SPPA asuransijogja.biz.id_`;
  ...
  if (submitted) {
    return (
      <section className="min-h-[calc(100vh-68px)] bg-cream flex items-center justify-center px-4 py-16">
        <div className="bg-white rounded-2xl shadow-lg border border-black/6 p-10 max-w-md w-full text-center">
          ...
          <h2 className="font-heading text-2xl font-bold text-navy mb-2">Permintaan Terkirim!</h2>
          <p className="text-sm text-[#64748B] leading-relaxed mb-6">
            Data Anda telah diteruskan ke WhatsApp kami. Tim konsultan akan segera menghubungi
            Anda — biasanya dalam 1×24 jam kerja.
          </p>
          <button
            onClick={() => {
              setSubmitted(false);
              setForm({ nama: "", whatsapp: "", agree: false });
              setProduct("");
            }}
            className="bg-navy text-white text-sm font-bold px-7 py-2.5 rounded-xl hover:bg-navy/85 transition-colors"
          >
  ...
  /* ── Main form ── */
  return (
    <section className="min-h-[calc(100vh-68px)] bg-cream py-12 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-[#64748B] max-w-[480px] mx-auto leading-relaxed">
            Isi form berikut untuk mendapatkan penawaran premi yang akurat.
            Konsultasi <strong className="text-navy">gratis</strong>, tanpa tekanan —
            kami bantu temukan solusi terbaik untuk Anda.
          </p>
        </div>
        ...
        <form
          ...
          className="bg-white rounded-2xl border border-black/6 shadow-[0_4px_40px_rgba(13,33,55,0.08)] overflow-hidden"
        >
          {/* Product selector — navy bar */}
          <div className="bg-navy px-6 py-5">
            ...
            <select
              ...
              className="w-full bg-white/10 border border-white/20 text-white rounded-lg px-3.5 py-2.5 text-sm font-medium focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/25 transition-all appearance-none cursor-pointer"
            >
              {PRODUCTS.map((p) => (
              ...
              <div className="bg-cream border border-[#D4C9B8]/70 rounded-xl p-4">
                <label className="flex items-start gap-3 cursor-pointer group">
                  ...
                  <span className="text-xs text-[#475569] leading-relaxed group-hover:text-navy transition-colors">
                    ...
                    <em>underwriting</em> lebih lanjut, khusus untuk risiko tinggi atau nilai
                    pertanggungan besar.
                  </span>
                </label>
              </div>
          ...
          {product && (
            <div className="px-6 pb-6">
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[#25D366] hover:bg-[#1db954] active:scale-[0.99] text-white font-bold text-sm py-3.5 rounded-xl transition-all hover:-translate-y-0.5 hover:shadow-lg disabled:opacity-60 disabled:translate-y-0 flex items-center justify-center gap-2.5"
              >
                {loading ? (
                  <>
              ...
              <p className="text-center text-[0.68rem] text-[#94A3B8] mt-2.5 leading-relaxed">
        ...
        {product && requirements && (
          <div className="mt-5 bg-white rounded-2xl border border-black/6 shadow-[0_4px_24px_rgba(13,33,55,0.05)] overflow-hidden">
            <div className="px-6 py-4 border-b border-black/6 flex items-center gap-3">
              <div className="w-8 h-8 bg-gold/10 rounded-lg flex items-center justify-center text-base flex-shrink-0">
                📎
              </div>
              <div>
                <div className="text-[0.65rem] font-bold tracking-[2px] uppercase text-[#94A3B8]">
                  Persiapan Dokumen
                </div>
            ...
            <div className="px-6 py-5">
              <p className="text-xs text-[#64748B] mb-4 leading-relaxed">
                Dokumen berikut umumnya dibutuhkan oleh pihak asuransi untuk proses{" "}
                <em>underwriting</em> dan penerbitan polis{" "}
              ...
              <div className="mt-5 bg-cream rounded-xl px-4 py-3 text-xs text-[#64748B] leading-relaxed border border-[#D4C9B8]/50">
                <strong className="text-navy">💡 Catatan:</strong> Dokumen dapat dikirim via WhatsApp
                setelah konsultasi awal. Anda <em>tidak perlu</em> menyiapkan semua dokumen
                sekarang — konsultasi pertama bisa dilakukan tanpa dokumen apapun.
              </div>
            </div>
          </div>
        )}

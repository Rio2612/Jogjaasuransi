export default function Why() {
  const cards = [
    ...
    { ico: "🏆", title: "9+ Tahun Pengalaman", desc: "Lebih dari sembilan tahun pengalaman di industri asuransi kerugian. Memahami produk, risiko, dan regulasi secara mendalam." },
  ...
  return (
    <section id="why" className="py-[5.5rem] px-[5vw] bg-white">
      <div className="text-center mb-12">
        <div className="text-xs font-bold tracking-[2.5px] uppercase text-goldtext mb-2">Keunggulan Kami</div>
        <h2 className="font-heading text-[clamp(1.9rem,3vw,2.9rem)] leading-[1.22] text-navy">Mengapa Pilih<br />Asuransi Jogja?</h2>
        <p className="text-[0.95rem] leading-[1.8] text-[#475569] max-w-[560px] mx-auto mt-3">Bukan sekadar agen penjual — kami adalah mitra perlindungan jangka panjang yang berpihak penuh pada kepentingan Anda.</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {cards.map(c => (
          <div key={c.title} className="bg-[#EEF3F9] rounded-card p-7 text-center hover:-translate-y-1 hover:shadow-[0_8px_28px_rgba(13,33,55,0.08)] transition-all">
            ...
            <p className="text-[1rem] leading-[1.7] text-[#475569]">{c.desc}</p>
          </div>
        ))}
      </div>
    </section>

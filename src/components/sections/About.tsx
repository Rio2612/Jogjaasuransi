    ...
    <section id="about" className="py-[5.5rem] px-[5vw] bg-cream">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-[4.5rem] items-center">
        ...
        <div>
          ...
          <p className="text-[0.95rem] leading-[1.8] text-[#64748B] max-w-[560px] mt-3">Dengan pengalaman lebih dari 10 tahun di bidang asuransi kerugian, Rio Mardiansyah hadir sebagai konsultan independen yang berkomitmen memberikan solusi perlindungan terbaik — bukan sekadar menjual polis.</p>
          <ul className="flex flex-col gap-3 mt-5">
            {[
              ...
              <li key={li} className="flex gap-2.5 items-start text-[0.88rem] leading-[1.65] text-[#64748B]">
                <span className="text-gold font-bold flex-shrink-0 mt-0.5">✓</span>{li}
              </li>
            ))}
          </ul>
          <div className="flex flex-col gap-2 mt-6">
            <a href={`https://wa.me/${KONTAK.wa}`} className="flex items-center gap-2 text-[0.88rem] text-navy2 font-medium no-underline hover:text-gold transition-colors">
              📱 {KONTAK.waDisplay} (WhatsApp)
            </a>
            <a href={`mailto:${KONTAK.email}`} className="flex items-center gap-2 text-[0.88rem] text-navy2 font-medium no-underline hover:text-gold transition-colors">
              ✉️ {KONTAK.email}
            </a>
            <span className="flex items-center gap-2 text-[0.88rem] text-navy2 font-medium">
              📍 {KONTAK.kota}
            </span>
          </div>
        </div>

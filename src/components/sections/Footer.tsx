    ...
    <footer className="bg-[#071422] px-[5vw] pt-12 pb-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 pb-8 border-b border-white/7">
        <div>
          ...
          <p className="text-[0.95rem] leading-[1.75] text-white/45">Konsultan asuransi kerugian independen di Yogyakarta. Melayani individu, UMKM, dan korporasi dengan solusi perlindungan yang tepat, transparan, dan dapat diandalkan.</p>
        </div>
        <div>
          ...
          <ul className="flex flex-col gap-2">
            {["Asuransi Kendaraan","Asuransi Properti","Engineering Insurance","Asuransi Kargo","Liability Insurance","Surety Bond"].map(p => (
              <li key={p}><a href="#produk" className="text-[0.95rem] text-white/50 no-underline hover:text-gold2 transition-colors">{p}</a></li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <ul className="flex flex-col gap-2">
            {[
              ...
              <li key={l.lbl}><a href={l.href} className="text-[0.95rem] text-white/50 no-underline hover:text-gold2 transition-colors">{l.lbl}</a></li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <ul className="flex flex-col gap-2">
            <li><a href={`https://wa.me/${KONTAK.wa}`} className="text-[0.95rem] text-white/50 no-underline hover:text-gold2 transition-colors">📱 {KONTAK.waDisplay}</a></li>
            <li><a href={`mailto:${KONTAK.email}`} className="text-[0.95rem] text-white/50 no-underline hover:text-gold2 transition-colors">✉️ {KONTAK.email}</a></li>
            <li><span className="text-[0.95rem] text-white/50">📍 Yogyakarta, DIY</span></li>
          </ul>
        </div>
      </div>

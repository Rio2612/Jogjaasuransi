  ...
  return (
    <section id="faq" className="py-[5.5rem] px-[5vw] bg-cream">
      <div className="text-center mb-12">
        <div className="text-xs font-bold tracking-[2.5px] uppercase text-goldtext mb-2">FAQ</div>
      ...
      <div className="max-w-[700px] mx-auto">
        {FAQS.map((faq, i) => (
          <div key={i} className="border-b border-black/8">
            ...
            >
              {faq.q}
              <span className={`text-goldtext text-[1.1rem] flex-shrink-0 transition-transform duration-300 ${openIdx === i ? "rotate-45" : ""}`}>+</span>
            </button>
            <div className={`text-sm leading-[1.78] text-[#475569] overflow-hidden transition-all duration-400 ${openIdx === i ? "max-h-[300px] pb-4" : "max-h-0"}`}>
              {faq.a}
            </div>
          </div>
        ))}
